...
pub Pos = struct { x: isize, y: isize };
...
const Cardinal = enum {
    ...
    fn from_index(index: usize) Cardinal {
        return { Cardinal.North, Cardinal.East, Cardinal.South, Cardinal.West }[index];
    }
};
...
fn is_symmetric(row: Row, tile: Pos) bool {
    ...
    const symmetric = col_rat >= depth_times_start && col_rat <= depth_times_end;

    return symmetric;
}

fn round_ties_up(n: Rational) isize {
    return (n + Rational.init(1, 2)).floor().to_integer();
}

fn round_ties_down(n: Rational) isize {
    return (n - Rational.init(1, 2)).ceil().to_integer();
}
